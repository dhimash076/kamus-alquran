        ...
        <div class="max-w-3xl mx-auto px-6">
            ...
            <div class="bg-white rounded-[2rem] p-8 md:p-10 overflow-hidden relative" style="border: 1.5px solid #e8d9b5; box-shadow: 0 10px 40px rgba(26,58,42,0.08);">
                ...
                <h1 class="text-3xl font-black uppercase tracking-tighter mb-2 mt-2" style="color: #1a3a2a;">
                    Edit <span style="color: #c9a84c;">Kosa Kata</span>
                </h1>
                <p class="text-[10px] font-black uppercase tracking-[0.4em] mb-8" style="color: #b0997a;">Perbarui Data Kosa Kata</p>
                ...
                @if ($errors->any())
                    <div class="rounded-2xl p-6 mb-8" style="background: #fff5f5; border: 1px solid #f5c6c6;">
                        <p class="font-black uppercase text-xs mb-2 tracking-widest" style="color: #c03030;">
                            <i class="fas fa-exclamation-triangle mr-1"></i> Gagal Memperbarui:
                        </p>
                ...
                <form action="{{ route('admin.update', $vocabulary->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="category_id" required
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('category_id', $vocabulary->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="arabic" value="{{ old('arabic', $vocabulary->arabic) }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="transliteration" value="{{ old('transliteration', $vocabulary->transliteration) }}" 
                        ...
                        <div>
                            ...
                            <input type="text" name="meaning" value="{{ old('meaning', $vocabulary->meaning) }}" 
                                class="w-full rounded-2xl p-4 font-bold outline-none transition-all"
                                style="border: 2px solid #f0e6cc; background: #fdf9f2; color: #1c2e1c;"
                                onfocus="this.style.borderColor='#c9a84c'; this.style.background='#fff';"
                                onblur="this.style.borderColor='#f0e6cc'; this.style.background='#fdf9f2';"
                    ...
                    <div>
                        ...
                        @if($vocabulary->video_path)
                            <div class="mb-4 rounded-2xl aspect-video overflow-hidden" style="background: #1a3a2a;">
                                <video class="w-full h-full object-cover" muted autoplay loop>
                                    <source src="{{ asset('storage/' . $vocabulary->video_path) }}" type="video/mp4">
                                </video>
                            </div>
                            <p class="text-[10px] font-bold mb-3 uppercase italic tracking-widest" style="color: #b0997a;">
                                * Biarkan kosong jika tidak ingin mengganti video ini.
                            </p>
                        @endif
